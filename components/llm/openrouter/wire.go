...
// toWireToolChoice maps the gantry tool choice to the OpenAI-compatible
// polymorphic tool_choice parameter: a bare mode string ("auto", "none",
// "required") or the forced-function object. nil (and unknown future modes)
// return nil so the field is omitted and the provider default (auto) applies.
// The mapping is mechanical — a forced tool with an empty Name is forwarded
// as-is and rejected loudly by the provider (HTTP 400 via checkStatus), never
// fixed up silently.
