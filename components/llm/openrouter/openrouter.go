...
// WithReasoningEffort sets OpenRouter's unified reasoning.effort request
// parameter ("low"/"medium"/"high", provider-defined; forwarded as-is, not
// validated here), which OpenRouter normalizes to whatever the underlying
// provider expects. Reasoning output — where the provider returns it — is
// surfaced to gantry as EventReasoningDelta / StreamChunk.ReasoningDelta via
// the response's reasoning field (see GenerateStream). An empty effort (the
