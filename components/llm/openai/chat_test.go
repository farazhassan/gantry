...
func newServerClient(t *testing.T, handler http.HandlerFunc) *openai.Client {
	t.Helper()
...
func TestGenerateMapsToolCallsPreservingIDs(t *testing.T) {
	...
	// Tools forwarded in OpenAI function wrapper.
	...
	// Response tool calls: ID preserved, name + raw arguments mapped.
...
func TestGenerateForwardsAssistantToolCallsAndToolResults(t *testing.T) {
	...
	_, err := c.Generate(context.Background(), harness.LLMRequest{
		Messages: []harness.Message{
			...
			{Role: harness.RoleTool, ToolCallID: "call_abc", Content: "72F"},
		},
	})
	if err != nil {
		t.Fatalf("Generate: %v", err)
