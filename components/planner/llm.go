...
func NewLLM(client harness.LLMClient, rubric string) *LLMPlanner {
	return &LLMPlanner{client: client, rubric: rubric}
}

func (p *LLMPlanner) Plan(ctx context.Context, task string) (*harness.Plan, error) {
	req := harness.LLMRequest{
		System: p.rubric,
		Messages: []harness.Message{
			{Role: harness.RoleUser, Content: task},
		},
	}
	...
	plan := &harness.Plan{Goal: task}
	for _, line := range strings.Split(resp.Content, "\n") {
		line = strings.TrimSpace(stripListMarker(line))
		if line == "" {
			continue
		}
		plan.Steps = append(plan.Steps, harness.PlanStep{Description: line})
	}
	return plan, nil
}
