...
func TestRegexGuardrailBlocksInputMatchingPattern(t *testing.T) {
	...
	state := &gantry.State{
		Messages: []gantry.Message{{Role: gantry.RoleUser, Content: "what is my password"}},
	}
...
func TestRegexGuardrailPassesClean(t *testing.T) {
	g := guardrail.NewRegex(`(?i)password`, guardrail.DirectionInput)
	state := &gantry.State{
		Messages: []gantry.Message{{Role: gantry.RoleUser, Content: "what is the weather"}},
	}
...
func TestRegexGuardrailChecksOutput(t *testing.T) {
	...
	state := &gantry.State{
		LastResponse: &gantry.LLMResponse{Content: "the secret is 42"},
	}
...
func TestRegexGuardrailDirectionMismatchSkips(t *testing.T) {
	...
	g := guardrail.NewRegex(`(?i)password`, guardrail.DirectionInput)
	state := &gantry.State{
		LastResponse: &gantry.LLMResponse{Content: "your password is 12345"},
	}
