...
// (a non-assistant turn tagged CriticAuthor) are skipped, so an in-flight
// tool call is never mistaken for the answer. Result is status-independent —
// the caller decides from the task's status whether the answer is final — and
// is safe on a nil task.
