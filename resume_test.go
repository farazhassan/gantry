...
func respWith(content string, in, out int) gantry.LLMResponse {
...
func TestRunFromCarriesTranscriptAndResetsScratch(t *testing.T) {
	...
	prior := gantry.NewState("old input")
	prior.Messages = []gantry.Message{
		{Role: gantry.RoleUser, Content: "first question"},
		{Role: gantry.RoleAssistant, Content: "first answer"},
	}
	prior.Usage = gantry.Usage{InputTokens: 5, OutputTokens: 5}
	...
	if got.Messages[2].Role != gantry.RoleUser {
	...
	if got.DoneReason != gantry.DoneNoToolCalls {
		t.Errorf("DoneReason = %q, want %q", got.DoneReason, gantry.DoneNoToolCalls)
	}
...
func TestRunFromMessagesAreIndependentOfPrior(t *testing.T) {
	...
	prior := gantry.NewState("x")
	prior.Messages = []gantry.Message{{Role: gantry.RoleUser, Content: "original"}}
	prior.Meta = map[string]any{"k": "v"}
...
func TestRunFromNilPriorEqualsRun(t *testing.T) {
	...
	a, _ := gantry.NewAgent(gantry.WithLLM(llmA))
	...
	b, _ := gantry.NewAgent(gantry.WithLLM(llmB))
