...
func TestRunFromStreamNilPriorMatchesRunStream(t *testing.T) {
	...
	state, err := a.RunFromStream(context.Background(), nil, "go", func(ev harness.Event) error {
...
func TestRunFromStreamCarriesPriorAndStreams(t *testing.T) {
	...
	prior := &harness.State{
		Messages: []harness.Message{
			{Role: harness.RoleUser, Content: "earlier question"},
			{Role: harness.RoleAssistant, Content: "earlier answer"},
		},
	}

	var sawDelta bool
	state, err := a.RunFromStream(context.Background(), prior, "follow up", func(ev harness.Event) error {
