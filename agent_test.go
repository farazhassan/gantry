...
func (nilLLM) Generate(ctx context.Context, req gantry.LLMRequest) (gantry.LLMResponse, error) {
	return gantry.LLMResponse{}, nil
}
...
func TestAgentUseRegistersMiddleware(t *testing.T) {
	...
	a.Use(gantry.PhaseLLMCall, mw)
	if got := a.MiddlewareCount(gantry.PhaseLLMCall); got != 1 {
		t.Errorf("MiddlewareCount = %d, want 1", got)
	}
	if got := a.MiddlewareCount(gantry.PhaseObserve); got != 0 {
		t.Errorf("unrelated phase count = %d, want 0", got)
	}
}

func TestWithMaxIterations(t *testing.T) {
	a, _ := gantry.NewAgent(gantry.WithLLM(nilLLM{}), gantry.WithMaxIterations(42))
...
func TestWithTracer(t *testing.T) {
	tr := gantry.NewTrace()
	a, err := gantry.NewAgent(gantry.WithLLM(nilLLM{}), gantry.WithTracer(gantry.NewDefaultTracer(tr)))
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	if a.Tracer() == nil {
...
func TestUseAppendsInOrder(t *testing.T) {
	a, _ := gantry.NewAgent(gantry.WithLLM(nilLLM{}))
	mk := func() gantry.Middleware {
		return func(next gantry.Handler) gantry.Handler { return next }
	}
	a.Use(gantry.PhaseLLMCall, mk())
	a.Use(gantry.PhaseLLMCall, mk())
	a.Use(gantry.PhaseLLMCall, mk())
	if got := a.MiddlewareCount(gantry.PhaseLLMCall); got != 3 {
		t.Fatalf("MiddlewareCount = %d, want 3", got)
	}
}

func TestWithLLMNilErrors(t *testing.T) {
	_, err := gantry.NewAgent(gantry.WithLLM(nil))
	if err == nil {
...
func TestWithMaxIterationsZeroErrors(t *testing.T) {
	_, err := gantry.NewAgent(gantry.WithLLM(nilLLM{}), gantry.WithMaxIterations(0))
	if err == nil {
...
func TestUseNamedRegistersWithName(t *testing.T) {
	a, _ := gantry.NewAgent(gantry.WithLLM(nilLLM{}))
	mw := func(next gantry.Handler) gantry.Handler { return next }
	a.UseNamed(gantry.PhaseLLMCall, "logger", mw)
	if got := a.MiddlewareCount(gantry.PhaseLLMCall); got != 1 {
		t.Errorf("MiddlewareCount = %d, want 1", got)
	}
	names := a.MiddlewareNames(gantry.PhaseLLMCall)
...
func TestUseBeforeInsertsBeforeAnchor(t *testing.T) {
	...
	mw := func(next gantry.Handler) gantry.Handler { return next }
	a.UseNamed(gantry.PhaseLLMCall, "logger", mw)
	a.UseBefore(gantry.PhaseLLMCall, "logger", "retry", mw)
	names := a.MiddlewareNames(gantry.PhaseLLMCall)
...
func TestUseAfterInsertsAfterAnchor(t *testing.T) {
	...
	mw := func(next gantry.Handler) gantry.Handler { return next }
	a.UseNamed(gantry.PhaseLLMCall, "logger", mw)
	a.UseAfter(gantry.PhaseLLMCall, "logger", "metrics", mw)
	names := a.MiddlewareNames(gantry.PhaseLLMCall)
...
func TestUseBeforeUnknownAnchorErrors(t *testing.T) {
	a, _ := gantry.NewAgent(gantry.WithLLM(nilLLM{}))
	mw := func(next gantry.Handler) gantry.Handler { return next }
	err := a.UseBefore(gantry.PhaseLLMCall, "ghost", "metrics", mw)
	if err == nil {
