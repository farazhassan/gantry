...
func TestDefaultTracerRecordsSpanStartAndEnd(t *testing.T) {
	tr := gantry.NewTrace()
	tracer := gantry.NewDefaultTracer(tr)
	...
	if events[0].Kind != gantry.KindSpanStart || events[0].Name != "phase:test" {
		t.Errorf("event[0] = %+v", events[0])
	}
	if events[1].Kind != gantry.KindEvent || events[1].Name != "midpoint" {
		t.Errorf("event[1] = %+v", events[1])
	}
	if events[2].Kind != gantry.KindSpanEnd || events[2].Err != nil {
